...
- `server.js`: Express API (OpenAI-compatible) exposing `GET /healthz`, `GET|HEAD|OPTIONS /v1/models`, and `POST /v1/chat/completions`. Streams SSE and maps `model: "codex-5"` → Codex `gpt-5`.
- `auth/server.mjs`: Lightweight ForwardAuth service for Traefik (`/verify`, `/healthz`).
- `docker-compose.yml`, `Dockerfile`: Container build/runtime.
- `systemd/`: User service unit (`codex-openai-proxy.service`).
- `config/`: Example Roo Code provider config.

## Build, Test, and Development Commands

- Local dev: `npm install && npm run start` (Node ≥ 18). Server at `http://127.0.0.1:${PORT:-11435}/v1`.
- Quick checks (curl):
  - Health: `curl -s 127.0.0.1:11435/healthz | jq .`
  - Models: `curl -s 127.0.0.1:11435/v1/models | jq .`
  - Chat (non‑stream): `curl -s 127.0.0.1:11435/v1/chat/completions -H "Authorization: Bearer $PROXY_API_KEY" -H 'Content-Type: application/json' -d '{"model":"codex-5","stream":false,"messages":[{"role":"user","content":"Say hello."}]}' | jq .`
- Docker: `docker compose up -d --build` (see compose labels and volumes for Codex binary/credentials).
...
- Models: advertise per environment — dev: `codev-5{,-low,-medium,-high,-minimal}` plus `codev-5.1-{L,M,H}` (map to `gpt-5.1`), prod: `codex-5{,-low,-medium,-high,-minimal}`; accept both prefixes everywhere; normalize to the effective (`gpt-5` or `gpt-5.1`) target before spawning Codex.
...
- When addressing code review feedback, append fixes as a new commit (do **not** amend or rewrite existing commits). Keeping review changes in their own commit preserves review context and avoids confusing force pushes.
...
These directives are mandatory for agents operating in this repository. They prevent production misconfiguration and ensure the right tests run for the right changes.
...
- Traefik runs as a host/systemd service in PROD. Therefore, ForwardAuth MUST be `http://127.0.0.1:18080/verify`.
- Never change ForwardAuth to `auth:8080` unless Traefik is explicitly containerized and on the same Docker network.
- If a change might impact routing/labels, you MUST run the production smoke after deploy.

3. Regressions to avoid

- Do not remove or reorder the following Traefik routers without explicit instruction: `codex-api`, `codex-preflight`, `codex-models`, `codex-health`.
- Keep `traefik.docker.network=traefik` in labels and ensure the app service is attached to the external `traefik` network.

4. Test selection policy
...
- Client “Load failed” with edge `/healthz` 404 = most likely bad ForwardAuth target or labels not applied.
- Timeouts/502 = origin unreachable or container down (check compose and service health).

7. Source of truth

- `docker-compose.yml` in this repo is authoritative for PROD routing/labels. Do not maintain alternative production compose files elsewhere.
