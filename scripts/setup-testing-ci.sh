...
let PORT; let child;
beforeAll(async () => {
  const server = await spawnServer({ PROXY_API_KEY: "test-sk-ci", CODEX_BIN: "scripts/fake-codex-jsonrpc.js", PROXY_USE_APP_SERVER: "true", CODEX_WORKER_SUPERVISED: "true", PROXY_PROTECT_MODELS: "false" });
  PORT = server.PORT;
  child = server.child;
});
afterAll(() => { stopServer(child); });
