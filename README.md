...
- Dev vs Prod model IDs: dev advertises `codev-5*` plus `codev-5.1-{L,M,H}` and `codev-5.2-{L,M,H}` (map to `gpt-5.1` / `gpt-5.2` low/medium/high); prod advertises `codex-5*`. Both prefixes are accepted everywhere.
...
- Dev also exposes `codev-5.1-{L,M,H}` and `codev-5.2-{L,M,H}` which map directly to `gpt-5.1` / `gpt-5.2` with implied low/medium/high reasoning effort (uppercase suffix is optional; IDs are case-insensitive).
...
- Dev advertises: `codev-5`, `codev-5-{low,medium,high,minimal}`, plus `codev-5.1-{L,M,H}` and `codev-5.2-{L,M,H}` which route to `gpt-5.1` / `gpt-5.2` automatically.
...
- Dev (advertised): `codev-5`, `codev-5-low`, `codev-5-medium`, `codev-5-high`, `codev-5-minimal`, plus `codev-5.1-{L,M,H}` and `codev-5.2-{L,M,H}` which normalize to `gpt-5.1` / `gpt-5.2` at low/medium/high reasoning effort.
