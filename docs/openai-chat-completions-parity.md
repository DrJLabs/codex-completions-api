...
- GitHub Actions run `keploy test` through the `keploy-dry-run` job. As of 2025-09-22 the invocation fails when the CLI emits `ERROR No test-sets found...`, guarding against silent skips. Snapshots reside under `test-results/chat-completions/keploy/test-set-0/tests/*.yaml`; replay still depends on resolving the memlock privilege tracked in `docs/bmad/issues/2025-09-20-keploy-memlock-privilege.md`.
- Manual dry-run evidence (2025-09-20): `keploy test --config-path config --path test-results/chat-completions --test-sets test-set-0` now reads the 2.x snapshots but fails when the CLI tries to start the app (`failed to set memlock rlimit: operation not permitted`). Logs, metrics stub, and version output live under `docs/bmad/qa/artifacts/3.8/`, documenting the need for CAP_IPC_LOCK (privileged container/self-hosted runner) before replay can succeed.
