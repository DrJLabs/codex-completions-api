...
1. `/readyz` and `/livez` reflect worker handshake/backoff states and expose restart/backoff metadata; integration tests cover crash, slow-start, and restart scenarios and assert readyz flips within the documented thresholds. _[Source: docs/epics.md#story-33-health-probe-robustness]_ _[Source: docs/sprint-artifacts/tech-spec-epic-3.md#acceptance-criteria-authoritative]_ _[Source: docs/architecture.md#decision-summary]_
...
3. Probes corroborate logs/metrics: readiness output aligns with worker telemetry (restart/backoff) and existing Prometheus gauges; runbooks describe how to validate alignment during smoke and incident drills. _[Source: docs/epics.md#story-33-health-probe-robustness]_ _[Source: docs/sprint-artifacts/tech-spec-epic-3.md#services-and-modules]_ _[Source: docs/architecture.md#observability]_ _[Source: docs/PRD.md#functional-requirements]_
...
- Probe logic must continue to source worker state from the supervisor so readiness mirrors handshake/backoff and restart counts; keep `/healthz`/`/readyz`/`/livez` wired in the main Express app. _[Source: docs/sprint-artifacts/tech-spec-epic-3.md#services-and-modules]_ _[Source: docs/architecture.md#decision-summary]_
...
- Metrics pipeline already exposes restart/backoff and stream gauges behind guarded `/metrics`; reuse those sources in probes to avoid duplicate state machines. _[Source: docs/sprint-artifacts/3-2-metrics-pipeline-for-app-server-path.md]_
- Keep readiness semantics consistent with the metrics guard and Traefik health checks so probes and scrapes agree on worker state. _[Source: docs/sprint-artifacts/3-2-metrics-pipeline-for-app-server-path.md]_
- File references to reuse: `src/services/metrics/index.js`, `src/routes/metrics.js`, `src/middleware/metrics.js`, `tests/integration/metrics.int.test.js`, `tests/unit/metrics.normalization.test.js`, `docs/app-server-migration/metrics-and-alerts.md`, `docs/app-server-migration/alerts/metrics-alerts.yaml`, `docs/app-server-migration/dashboards/observability-dashboard.json`. _[Source: docs/sprint-artifacts/3-2-metrics-pipeline-for-app-server-path.md#file-list]_
- Completion notes/warnings from Story 3-2: metrics guard, smoke scrapes, dashboards/alerts, and restart/backoff gauges are already implemented—reuse those patterns and avoid duplicating state machines. _[Source: docs/sprint-artifacts/3-2-metrics-pipeline-for-app-server-path.md#completion-notes-list]_

### Project Structure Notes

- Probe handlers live in `src/routes/health.js` and consume supervisor snapshots; avoid moving them out of the main app to preserve Traefik/Compose expectations. _[Source: docs/stories/1-5-wire-readiness-and-liveness-probes-to-worker-state.md]_
- Supervisor state and restart/backoff data come from `src/services/worker/supervisor.js`; any new fields for probes must stay aligned with metrics emission to prevent divergence. _[Source: docs/stories/1-5-wire-readiness-and-liveness-probes-to-worker-state.md]_
...
- docs/sprint-artifacts/3-3-health-probe-integration-tests.context.xml
