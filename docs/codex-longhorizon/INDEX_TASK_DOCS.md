...
- Acceptance criteria (PROPOSED): Align readiness guard for `/v1/responses` with chat; ensure OPTIONS emits CORS headers; require bearer (or explicit edge-only guard) for `/v1/usage*`; include `/v1/responses` in rate-limit scope; document expected behavior for legacy `/v1/completions`.
...
- Summary: Details `normalizeChatJsonRpcRequest`; notes loss of message ordering/roles (system/user flattening), duplication between `turn` and `message`, permissive passthrough of tools/response_format, mixed validation/business logic, and ignored knobs (`requestedModel`, `choiceCount` usage). Identifies high-risk parity gaps and proposes tests.
- Acceptance criteria (PROPOSED): Make mapping deterministic and documented (or preserve roles/order); tighten validation for tools/response_format/reasoning; remove or document ignored fields; add tests for n>1, reasoning_effort, final_output schemas, and tool payloads; keep turn/message fields consistent.
- Priority clues: P1 correctness/maintainability.
