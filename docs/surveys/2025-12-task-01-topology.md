...
| ForwardAuth service    | `auth/`       | microservice   | Traefik ForwardAuth implementation (`server.mjs` and `server.js`) for API key auth.                   | mixed          | Duplicate entrypoints (mjs vs js) suggest legacy/alt build target and potential drift.             |       |
