...
> Update (2025-12-18): DRIFT-04 resolved by removing `PROXY_STREAM_MODE` from Dockerfile/compose/systemd/README and the archived install script.
...
- Stream mode confusion:
  - README called `PROXY_STREAM_MODE` “deprecated/no effect” yet documented `jsonl` behavior.
  - `docker-compose.yml` and `docs/_archive/install.sh` previously set it (removed 2025-12-18).
