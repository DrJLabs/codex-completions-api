...
- ForwardAuth canonicalized to `auth/server.mjs`; `auth/server.js` now exits unless `ALLOW_LEGACY_AUTH=true`, and README references were updated to the canonical entrypoint.
...
## Gaps
- Optional: add a CI/doc lint to assert `auth/server.js` is never referenced in manifests (runtime guard now exits by default).
...
- AC1: Select and mark the canonical ForwardAuth entrypoint; remove or clearly deprecate the other. Test layer: docs + static check. Implementation: annotate chosen file (`auth/server.mjs` or `auth/server.js`), update `docker-compose.yml`/`systemd/*.service` and README to reference only the canonical path; add a CI grep or comment to block the non-canonical file.
