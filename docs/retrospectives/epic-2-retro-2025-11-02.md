...
Addendum (2025-11-19):
- Story 2.11 (end-to-end tracing) is now fully approved; ingress logging and failure-path usage logging cover all exits, and `/v1/usage` remains joinable with trace events.
- Story 2.12 (stream tool-call buffering) is complete with production-visible `tool_buffer_*` counters via `/v1/usage` and a replayed Codex transcript regression; duplicate textual `<use_tool>` frames are resolved.
