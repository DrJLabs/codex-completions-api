...
- Next steps:
  - Stabilize streaming concurrency integration test (open QA issue: concurrency-guard flakiness).
  - Define and track production perf targets with real backend; keep perf smoke thresholds updated.
  - Design distributed rate limiting at edge/Redis (follow-up issue) and add tests.
