...
4. Tool‑block behavior parity (no regressions):
   ...
   - `PROXY_SUPPRESS_TAIL_AFTER_TOOLS` preserves current semantics for both stream and non‑stream.
...
- Tool‑block behavior:
  ...
  - Tail suppression: `PROXY_SUPPRESS_TAIL_AFTER_TOOLS` truncates post‑tool content.
