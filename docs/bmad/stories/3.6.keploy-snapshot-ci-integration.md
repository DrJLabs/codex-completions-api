...
Ready for Review — 2025-09-20 (dev validation complete)
...
- Keploy 3.0 introduces instrumentation-free snapshot capture via the `keploy` proxy and config-driven `keploy.yml`, enabling selective recording, sanitization filters, and record/test toggles that align with CI pipelines. citeturn0search0turn0search1turn1search0
- GitHub Actions guidance demonstrates running Keploy in dedicated record/test jobs with explicit `KEPLOY_MODE` settings, isolated service containers, and artifact uploads for reports, informing our workflow design. citeturn0search2
...
6. CI pipelines define explicit Keploy record and test stages (with cacheable artifacts and report uploads) following current GitHub Actions best practices for containerized services. citeturn0search2
...
- [x] Provision Keploy CLI in CI and dev environments (AC 1)
  ...
  - [x] Use the official Keploy installer or containerized proxy (`keploy proxy --config keploy.yml`) to provision binaries and background services consistently across runners. citeturn0search0turn0search1
...
- [x] Enhance transcript generation tooling for Keploy (AC 2)
  ...
  - [x] Version a `keploy.yml` that defines the proxy service command, target ports, and selective recording toggles used during transcript capture. citeturn0search0turn1search0
...
- [x] Capture and manage Keploy snapshots (AC 3)
  ...
  - [x] Configure masking/sanitization rules in `keploy.yml` so sensitive headers, tokens, and IDs are scrubbed before committing fixtures. citeturn1search0
...
- [x] Wire contract suites to Keploy replays (AC 4)
  ...
  - [x] Align GitHub Actions jobs with the recommended record → test pattern, publishing Keploy reports/artifacts for debugging. citeturn0search2
...
- Component Specifications:
  ...
  - Keploy 3.0 runs as a proxy defined in `keploy.yml`, so CI jobs must launch the proxy alongside the Node service and respect record/test toggles. citeturn0search0turn0search1
- File Locations:
...
- Testing Requirements:
  ...
  - Adopt the record → test job pattern recommended for GitHub Actions, archiving Keploy reports for debugging CI failures. citeturn0search2
...
- Completion Notes:
  ...
  - 2025-09-20: Added `KEPLOY_ENABLED` flow in transcript tooling, generated YAML snapshots, and introduced `config/keploy.yml` + launch wrapper.
