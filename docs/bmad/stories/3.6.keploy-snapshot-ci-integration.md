...
- Resolved by `docs/bmad/issues/2025-09-22-keploy-dry-run-replay-gap.md`: snapshots now sit under `test-results/chat-completions/keploy/test-set-0/tests`, and the CI dry-run step fails whenever Keploy surfaces `No test-sets found` or other errors. Replay remains blocked on the memlock privilege tracked in `docs/bmad/issues/2025-09-20-keploy-memlock-privilege.md`.
