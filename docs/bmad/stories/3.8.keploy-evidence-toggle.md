...
- [ ] Validate local/dev stack evidence (AC 2)
  - [ ] Run `./scripts/setup-keploy-cli.sh` and `keploy test --config-path config/keploy.yaml` locally, recording port binding checks for 16789/16790/26789 on loopback and noting any constraints from Keploy’s limited test-set handling. [Source: docs/bmad/architecture/tech-stack.md#Testing & QA; citeturn0search3citeturn1search9]
...
- **Testing Requirements**: `npm run verify:all` remains the umbrella command; enabling Keploy routes integration/E2E suites through `keploy test --config-path config/keploy.yaml`. [Source: docs/bmad/architecture/tech-stack.md#Testing & QA]
...
- `keploy test --config-path config/keploy.yaml` (local) to validate snapshot replay before capturing evidence. [Source: docs/bmad/architecture/tech-stack.md#Testing & QA]
