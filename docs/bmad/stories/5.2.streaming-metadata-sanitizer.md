...
- Documented `PROXY_SANITIZE_METADATA` streaming coverage in `.env.example` and `infra/compose/docker-compose.local.example.yml` for rollout guidance.
...
- Configuration and docs (`.env.example`, `infra/compose/docker-compose.local.example.yml`) describe the toggle so operators can stage rollout safely; no regressions spotted outside the streaming path.
