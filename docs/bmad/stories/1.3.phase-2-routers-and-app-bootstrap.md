...
- [x] Create `src/app.js` that:
  - [x] Sets up `express.json({ limit: "16mb" })`.
  - [x] Applies global CORS with the existing util and preflight handling.
  - [x] Applies structured access log middleware (kept from Phase 1).
  - [x] Mounts routers exported by `src/routes/*`.

- [x] Extract health router → `src/routes/health.js`:
  - [x] `GET /healthz` returns `{ ok: true, sandbox_mode }` exactly as today.

- [x] Extract models router → `src/routes/models.js`:
  - [x] `GET /v1/models` returns the same payload and headers; keep `Cache-Control: public, max-age=60`.
  - [x] `HEAD /v1/models` and `HEAD /v1/models/` return `200` with same `Content-Type`.
  - [x] `OPTIONS /v1/models` and `OPTIONS /v1/models/` return `Allow: GET,HEAD,OPTIONS`.
  - [x] When `PROXY_PROTECT_MODELS=true`, require Bearer key and return PRD‑compliant 401 with `WWW-Authenticate` header.

- [x] Update `server.js` to:
  - [x] Import `app` from `src/app.js`.
  - [x] Only read port and start the HTTP server (signals unchanged); remove in‑file health/models logic.

- [x] Verify smoke tests (PRD) and headers:
  - [x] `curl -s $BASE/healthz | jq .` → 200 with `{ ok, sandbox_mode }`.
  - [x] `curl -sI $BASE/v1/models` → 200 + correct headers; gating verified when enabled.
...
| 2025-09-11 | 1.0     | Implemented, verified, and merged (PR #57) | dev/qa |
