...
- **Tools contract:** `tools` must be an array of function tools. `tool_choice` accepts `auto`, `none`, `required`, or `{ type: "function", function: { name } }` that references a declared tool. `parallel_tool_calls` must be boolean when provided.
