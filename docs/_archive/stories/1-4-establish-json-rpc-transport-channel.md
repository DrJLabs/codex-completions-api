...
- Extend the transport inside `src/services/` alongside the worker supervisor delivered in Story&nbsp;1.3 so lifecycle coordination stays co-located. [Source](docs/bmad/architecture/source-tree.md#src-modules) [Source](docs/_archive/stories/1-3-implement-worker-supervisor-and-lifecycle-hooks.md#project-structure-alignment)
...
- [x] (AC #1) Implement JSON-RPC client bootstrap (`initialize`, `sendUserTurn`, `sendUserMessage`) and refuse requests until handshake succeeds. [Source](docs/app-server-migration/codex-completions-api-migration.md#c-json-rpc-client-write--read)
  - [x] (AC #1) Consume supervisor readiness signals so adapters dispatch only after advertised models arrive. [Source](docs/_archive/stories/1-3-implement-worker-supervisor-and-lifecycle-hooks.md#dev-notes)
...
- Uphold the supervised single-channel model from Story 1.3—transport readiness must mirror supervisor state rather than spinning parallel connections. [Source](docs/_archive/stories/1-3-implement-worker-supervisor-and-lifecycle-hooks.md#dev-notes)
...
- **New Service Available:** `src/services/worker/supervisor.js` now manages lifecycle; transport should consume its readiness hooks instead of duplicating state. [Source](docs/_archive/stories/1-3-implement-worker-supervisor-and-lifecycle-hooks.md#file-list)
- **Structural Change:** Chat handlers already gate traffic on supervisor readiness—extend those hooks rather than introducing new switches. [Source](docs/_archive/stories/1-3-implement-worker-supervisor-and-lifecycle-hooks.md#dev-notes)
...
- **Operational Expectation:** Supervisor logs restart warnings and metrics; transport should surface retryable worker events through the same telemetry. [Source](docs/_archive/stories/1-3-implement-worker-supervisor-and-lifecycle-hooks.md#debug-log-references)
...
- [x] 2025-10-31: Story context generated and validation report saved at docs/_archive/validation-reports/validation-report-2025-10-31T123730Z.md.
...
- docs/_archive/story-contexts/1-4-establish-json-rpc-transport-channel.context.xml
