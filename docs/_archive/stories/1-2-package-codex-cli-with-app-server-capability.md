...
- Reuse `src/services/backend-mode.js` plus the new unit/integration suites (`tests/unit/config/backend-mode.spec.js`, `tests/integration/backend-mode.int.test.js`) to centralize backend selection logic instead of re-reading env flags in Docker scripts. [(Source)](docs/_archive/stories/1-1-add-app-server-feature-flag-scaffold.md#completion-notes-list) [(Source)](docs/_archive/stories/1-1-add-app-server-feature-flag-scaffold.md#file-list)
- Keep rollout documentation and env samples synchronized; the docs lint added in Story 1.1 will fail if defaults drift. [(Source)](docs/_archive/stories/1-1-add-app-server-feature-flag-scaffold.md#debug-log-references)
- No outstanding review tasks remain; continue the approved approach to logging and spawn argument selection. [(Source)](docs/_archive/stories/1-1-add-app-server-feature-flag-scaffold.md#senior-developer-review-ai)
...
- Respect the non-root runtime by preparing writable directories before `USER node`, mirroring Story 1.1’s handling of shared helpers. [(Source)](docs/_archive/stories/1-1-add-app-server-feature-flag-scaffold.md#architecture--structure-alignment)
...
- docs/research-technical-2025-10-30.md#option-1-codex-app-server-migration
- docs/_archive/stories/1-1-add-app-server-feature-flag-scaffold.md#completion-notes-list
...
- docs/_archive/story-contexts/1-2-package-codex-cli-with-app-server-capability.context.xml
...
- Packaging (AC#1) — Pinned `@openai/codex` to 0.53.0, updated the migration guide, and added a Dockerfile guard that runs `codex app-server --help` during the build; validated the baked binary with `docker run --rm codex-completions-api:latest codex --version`.
...
- [x] 2025-10-31: Story context generated (docs/_archive/story-contexts/1-2-package-codex-cli-with-app-server-capability.context.xml).
- [x] 2025-10-31: Context validation report saved (docs/_archive/validation-reports/validation-report-2025-10-31T052130Z.md).
- [x] 2025-10-31: Context validation re-run (docs/_archive/validation-reports/validation-report-2025-10-31T052520Z.md).
- [x] 2025-10-31: Independent validation report generated (docs/_archive/validation-reports/validation-report-2025-10-31T051932Z.md).
