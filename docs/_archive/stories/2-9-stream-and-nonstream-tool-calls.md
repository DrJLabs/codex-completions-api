...
- `docs/_archive/story-contexts/2-9-stream-and-nonstream-tool-calls.md.context.xml`
...
1. **High – Streaming tool-call regression test never landed.** Story tasks call for `tests/integration/chat.stream.tool-calls.int.test.js`, but only the documentation mentions that filename (`docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:38-47`); the `tests/integration` tree contains no such suite, leaving `<use_tool>` streaming parity unguarded.
2. **High – Non-stream + finish-reason unit coverage missing.** The plan lists `tests/unit/handlers/chat/nonstream.test.js` and `tests/unit/lib/finish-reason.test.js` (`docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:40-47`), yet neither file exists, so non-stream envelope changes and finish reason precedence lack any targeted tests.
3. **Medium – Tool-call E2E spec absent.** AC6’s test bullet promises `tests/e2e/tool-calls.spec.ts` (`docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:48-50`), but the Playwright suite only contains the existing contract specs, meaning there’s no end-to-end evidence for mixed textual/structured flows.
...
| 4 | Implemented | `src/handlers/chat/shared.js:196-258`; `src/handlers/chat/stream.js:903-949`; `src/handlers/chat/nonstream.js:205-213` (unit test pledged in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:44-45` still missing) |
...
| 6 | Partial | Requirement recorded in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:48-50`, but the Playwright suite (`tests/e2e/chat-contract.spec.js`, `tests/e2e/chat-stream-metadata.spec.ts`, etc.) has no `tool-calls.spec.ts`, so there is no E2E coverage for the promised scenarios. |
...
| AC #6 – Test coverage depth | Partial | Integration/E2E coverage promised in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:48-50` lacks the new Playwright spec. |
...
| Task | Status | Evidence |
| --- | --- | --- |
| Testing – AC #1 (`tests/integration/chat.stream.tool-calls.int.test.js`) | **Not Done** | File never created; only the story text references it (`docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:38-41`). |
| Testing – AC #2 (`tests/unit/handlers/chat/nonstream.test.js`) | **Not Done** | Path does not exist anywhere in `tests/unit`; requirement documented at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:40-41`. |
...
| Testing – AC #4 (`tests/unit/lib/finish-reason.test.js`) | **Not Done** | No such suite under `tests/unit`; task recorded at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:44-45`. |
| Testing – AC #5 (`tests/integration/chat.nonstream.tool-calls.int.test.js`) | **Not Done** | Missing from `tests/integration`; only referenced in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:46-47`. |
| Testing – AC #6 (`tests/e2e/tool-calls.spec.ts`) | **Not Done** | Playwright folder lacks this file; requirement documented in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:48-50`. |
| Testing – AC #7 (duplicate delta simulation) | Questionable | No test mentions duplicate backend deltas (repo search for "duplicate" under `tests/` returned no matches); requirement lives at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:50-51`. |
| Testing – AC #8 (UTF-8 fixtures) | **Not Done** | There are no tests containing "emoji"/"CJK" keywords; obligation noted in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:52-53`. |
...
| Testing – AC #10 (post-finish callbacks) | Questionable | Requirement at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:56-57`; no integration test invokes backend events after finish. |
| Testing – AC #11 (non-stream parity snapshots) | Verified | `tests/integration/chat.contract.nonstream.int.test.js:63-87`. |
| Testing – AC #12 (no mixed frames) | Questionable | No tests assert mutually exclusive `content` vs `tool_calls`; requirement noted at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:60-61`. |
| Testing – AC #13 (SSE header supertest) | **Not Done** | No test inspects `X-Accel-Buffering`; requirement documented in `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:62-63`. |
| Testing – AC #14 (pre/post tool-call error fixtures) | Questionable | Error suites exist but none exercise tool-call timing; requirement lives at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:64-65`. |
| Testing – AC #15 (large-arg streaming) | **Not Done** | No test suite streams large UTF-8 arguments; requirement at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:66-67`. |
...
| Testing – AC #19 (function_call vs tool_calls precedence) | **Not Done** | No integration/unit suite crafts payloads containing both shapes; requirement at `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:74-75`. |
| Testing – AC #20 (mixed input shapes) | Questionable | There is no unit test demonstrating Codex v2 + OpenAI deltas flowing together; see `docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:76-77`. |
...
- ❌ **Testing – AC #5** (docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:46-49): there is still no `tests/integration/chat.nonstream.tool-calls.int.test.js`, and no config unit test covers `PROXY_OUTPUT_MODE`.
- ❌ **Testing – AC #8 / #15** (docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:52-67): existing suites only exercise ASCII payloads (`tests/unit/tool-call-aggregator.test.ts:10-56`), so the required UTF-8/large-argument regressions remain unmapped.
- ❌ **Testing – AC #13** (docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:62-63): no test asserts SSE headers or `res.flush()` usage; streaming specs parse bodies only.
- ❌ **Testing – AC #17** (docs/_archive/stories/2-9-stream-and-nonstream-tool-calls.md:70-71): the disconnect test is skipped (`tests/integration/kill-on-disconnect.int.test.js:51-86`), so the checkbox cannot be considered complete.
