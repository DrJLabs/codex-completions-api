...
test('404 model_not_found for unknown model', async () => {
  const r = await fetch(`${BASE}/chat/completions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${API_KEY}` },
    body: JSON.stringify({ model: 'does-not-exist', stream: false, messages: [{ role: 'user', content: 'hi' }] }),
  });
...
test('chat completions non-stream returns assistant text', async () => {
  const r = await fetch(`${BASE}/chat/completions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${API_KEY}` },
    body: JSON.stringify({ model: 'codex-5', stream: false, messages: [{ role: 'user', content: 'Say hello' }] }),
  });
  expect(r.status).toBe(200);
  const j = await r.json();
...
test('usage endpoints produce aggregates', async () => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await fetch(`${BASE}/chat/completions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${API_KEY}` },
      body: JSON.stringify({ model: 'codex-5', stream: false, messages: [{ role: 'user', content: 'Say hello again' }] }),
    });
  }
