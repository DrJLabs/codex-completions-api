/*
 * security/detect-object-injection is disabled for this module because the aggregator must
 * inspect dynamic properties on JSON-RPC payloads emitted by the Codex worker. The upstream
 * schema includes optional objects whose shape is not known at lint time, so we intentionally
 * access properties via bracket notation. Each access is gated by explicit type checks, keeping
...
function extractArgumentFragment(payload = {}) {
  ...
  const fnPayload =
    payload.function && typeof payload.function === "object" ? payload.function : null;
  if (fnPayload) {
...
export function createToolCallAggregator({ idFactory } = {}) {
  ...
  const ingestInternal = (
    payload,
    { mode, emit = true, fallbackEmit = false, choiceIndex = 0 } = {}
  ) => {
  ...
  return {
    ...
    snapshot(options = {}) {
      ...
      return choiceState.order
        .map((key) => cloneRecord(choiceState.callsByKey.get(key)))
        .map((record) => ({
          id: record.id,
          type: record.type,
