...
if (process.env.DISABLE_SERVER_POSTS === "1")
  app.post("/v1/chat/completions", (req, res) => {
    ...
    if (isStreamingReq) {
      ...
      const keepaliveMs =
        disableKeepaliveUA || disableKeepaliveHeader || disableKeepaliveQuery
          ? 0
          : SSE_KEEPALIVE_MS;
        ...
        try {
          ...
          if (tp === "agent_message_delta") content += String((evt.msg?.delta ?? evt.delta) || "");
            ...
            if (!responded) {
              ...
              try {
                child.stdin.write(
                  JSON.stringify({ id: nanoid(), op: { type: "shutdown" } }) + "\n"
                );
              } catch {}
              try {
                child.kill("SIGTERM");
